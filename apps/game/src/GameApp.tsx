...
export const GameApp: React.FC = () => {
  ...
  else if (isResult) content=<CombatResultContainer resolution={runtime.activeCombatResolution!} items={runtime.itemDefinitions} resolveItemName={runtime.entityNames.item} onTakeLoot={runtime.takeLoot} onExecutePostCombatAction={runtime.executePostCombatAction} onDismiss={runtime.dismissCombatResult} onLoadLastSave={()=>runtime.loadFromLocalSlot('autosave-1').success} onMainMenu={()=>setShowMainMenu(true)}/>;
  ...
  else if (isScreen && gameState.world.activeScreen==='Workbench') content=<CraftingScreen recipes={runtime.recipes} availableIds={runtime.availableRecipeIds} inventory={gameState.player.inventory} items={runtime.itemDefinitions} resolveItemName={runtime.entityNames.item} onCraft={runtime.craftRecipe} onReturn={runtime.returnToOrigin}/>;
  ...
  if (showCreator && runtime.characterCreationOptions.definition) return <CharacterCreator definition={runtime.characterCreationOptions.definition} races={runtime.characterCreationOptions.races} classes={runtime.characterCreationOptions.classes} backgrounds={runtime.characterCreationOptions.backgrounds} perks={runtime.characterCreationOptions.perks} abilities={runtime.combatAbilities} items={runtime.itemDefinitions} randomizeName={runtime.generateCharacterName} adjustAttribute={runtime.adjustCharacterCreationAttribute} adjustSkill={runtime.adjustCharacterCreationSkill} validate={runtime.validateCharacterCreation} preview={runtime.previewCharacterCreation} onCancel={()=>setShowCreator(false)} onConfirm={(selection)=>{const result=runtime.initializeNewGame(selection);if(result.valid){setShowCreator(false);setShowMainMenu(false)}}}/>;
  ...
  return <GameShell mode={shellMode} hud={hud}>{content}
    {mode==='ActionResult'&&runtime.activeActionResolution&&<ActionResultModal resolution={runtime.activeActionResolution} items={runtime.itemDefinitions} resolveItemName={runtime.entityNames.item} onDismiss={runtime.dismissActionResolution}/>}
    {mode==='Dialogue'&&runtime.activeDialogueState&&<SceneEventScreen mode="dialogue" dialogueState={runtime.activeDialogueState} onChoose={runtime.chooseDialogueOption} onAdvancePlayerLine={runtime.advanceDialoguePlayerLine} onAdvanceNode={runtime.advanceDialogueNode} onClose={runtime.endDialogue}/>}
    {showCharacter&&<CharacterSheet state={gameState} resolvedPlayer={runtime.resolvedPlayer} playerRace={runtime.playerRace} playerBackground={runtime.playerBackground} raceNames={runtime.raceNames} classNames={runtime.classNames} resolveAbilityName={runtime.entityNames.ability} primaryClass={runtime.primaryClass} specialPaths={runtime.specialPaths} items={runtime.itemDefinitions} perks={runtime.characterCreationOptions.perks} progression={runtime.playerProgression} equipmentOptions={runtime.equipmentOptions} equipmentSlots={runtime.equipmentSlots} quests={runtime.questDossiers} party={runtime.partyMembers} onClose={()=>setShowCharacter(false)} onEquip={runtime.equipInventoryEntry} onUnequip={runtime.unequipSlot} onUse={runtime.useInventoryItem} onDrop={runtime.dropInventoryItem} onEquipCompanion={runtime.equipRosterCharacterItem} onUnequipCompanion={runtime.unequipRosterCharacterSlot} onTransferItem={runtime.transferRosterItem}/>}
