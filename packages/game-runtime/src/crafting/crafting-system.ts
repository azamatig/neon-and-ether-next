...
export class CraftingSystem {
  ...
  constructor(private readonly content:ContentRegistry,private readonly conditions:ConditionRegistry=defaultConditionRegistry,private readonly effects:EffectExecutor=defaultEffectExecutor){this.inventory=new InventorySystem(content);}
  ...
  canCraft(recipe:RecipeDefinition,state:GameState,context:CraftingContext):CraftingResult {
    ...
    const evaluated=evaluateConditions(recipe.conditions,{state,contentRegistry:this.content},this.conditions);if(!evaluated.allMet)return fail(evaluated.failedConditions[0]?.reason??'Crafting conditions are not met.');
  ...
  craft(recipeId:string,state:GameState,context:CraftingContext):CraftingResult {
    ...
    this.effects.executeBatch(recipe.effects,{state,contentRegistry:this.content});
