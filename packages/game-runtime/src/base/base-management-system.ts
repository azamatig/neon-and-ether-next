...
export class BaseManagementSystem {
  ...
  public getRoomOptions(slotId: string, state: GameState): BaseOption<BaseRoomDefinition>[] {
    ...
    return this.content.getAllRooms().map((definition) => {
      ...
      const conditionResult = evaluateConditions([...definition.requirements, ...definition.conditions], { state, contentRegistry: this.content, rollRandom:(min,max)=>this.random.integer(min,max) }, this.conditions);
  ...
  private buildRoom(slotId: string, definitionId: string, instanceId: string, state: GameState): BaseManagementResult {
    ...
    this.effects.executeBatch(definition.effects, { state, contentRegistry: this.content,random:this.random });
