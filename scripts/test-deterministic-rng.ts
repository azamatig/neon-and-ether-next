...
const run=(seed:number)=>{const session=new GameSession(registry,seed);const conditions=Array.from({length:8},()=>session.evaluateCondition({type:'randomChance',probability:.5}).isMet);expect(session.startCombatEncounter('enc_glassline_transit_standoff',false)&&session.startTacticalCombat(),'seeded tactical combat did not start');const combatBefore=session.getState().combat;const actorId=combatBefore.turnOrder[combatBefore.activeTurnIndex];const targetId=Object.values(combatBefore.combatants).find(unit=>unit.team==='Enemy')!.id;const ranged=session.getResolvedCombatCommands().actions.find(action=>action.type==='WeaponAttack'&&!action.disabledReason);const melee=session.getResolvedCombatCommands().actions.find(action=>action.type==='MeleeAttack'&&!action.disabledReason);const attack=ranged??melee;expect(attack,'seeded actor has no resolved attack');const command=session.executeCombatAction(attack!.type==='WeaponAttack'?{type:'RangedAttack',actorId,targetId,weaponId:attack!.weaponId!}:{type:'MeleeAttack',actorId,targetId,weaponId:attack!.weaponId});expect(command.success,'seeded attack failed');const damage={targetHp:command.state.combatants[targetId].currentHp,log:command.state.log.map(entry=>entry.message)};const combat=projection(session.resolveCombatVictory('enc_glassline_annex_core',3));return{conditions,damage,combat,rng:session.getRandomState()};};
