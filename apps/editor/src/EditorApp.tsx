...
type Category = 'dialogues' | 'minigames' | 'backgrounds' | 'races' | 'classes' | 'specialPaths' | 'namePools' | 'items' | 'npcs' | 'enemies' | 'factions' | 'pois' | 'events' | 'quests' | 'maps' | 'encounters' | 'rooms' | 'bases' | 'baseUpgrades' | 'baseJobs' | 'recipes' | 'shops' | 'weatherDefinitions' | 'weatherProfiles';
type EditableEntity = GameContent[Category][number];
const entityName = (entity: EditableEntity): string => 'name' in entity ? String(entity.name) : String((entity as DialogueTree).title);

const CATEGORY_LABELS: Record<Category, string> = { dialogues:'Dialogues', minigames:'Minigames', backgrounds: 'Backgrounds', races:'Races', classes:'Classes', specialPaths:'Special Paths', namePools:'Name Pools', items: 'Items', npcs: 'NPCs', enemies: 'Enemies', factions:'Factions', pois: 'POIs', events: 'Events', quests: 'Quests', maps: 'Maps', encounters: 'Encounters', rooms: 'Base Rooms', bases: 'Bases', baseUpgrades: 'Upgrades', baseJobs: 'Base Jobs', recipes: 'Recipes', shops: 'Shops', weatherDefinitions: 'Weather', weatherProfiles: 'Profiles' };
const CATEGORY_SCHEMAS = { dialogues:DialogueTreeSchema, minigames:MinigameDefinitionSchema, backgrounds: BackgroundDefinitionSchema, races:RaceDefinitionSchema, classes:ClassDefinitionSchema, specialPaths:SpecialPathDefinitionSchema, namePools:NamePoolDefinitionSchema, items: ItemSchema, npcs: NPCSchema, enemies: EnemySchema, factions:FactionSchema, pois: POISchema, events: GameEventSchema, quests: QuestSchema, maps: GameMapSchema, encounters: CombatEncounterSchema, rooms: BaseRoomDefinitionSchema, bases: PlayerBaseDefinitionSchema, baseUpgrades: BaseUpgradeDefinitionSchema, baseJobs: BaseJobDefinitionSchema, recipes: RecipeSchema, shops: ShopDefinitionSchema, weatherDefinitions: WeatherDefinitionSchema, weatherProfiles: WeatherProfileSchema } as const;
...
function createTemplate(category: Category, id: string): EditableEntity {
  ...
  if(category==='dialogues')return{id,title:'New Conversation',description:'',tags:[],rootNodeId:'opening',nodes:{opening:{id:'opening',speakerName:'NPC',text:'Opening line.',choices:[]}}} as EditableEntity;
  ...
  if(category==='races')return{id,name:'New Race',description:'',tags:[],availability:'NPC',rarity:'uncommon',requirements:[],attributeModifiers:[],skillModifiers:{},grantedTraits:[],grantedAbilityIds:[],startingEffects:[],parentRaceIds:[],lineageMetadata:{}} as EditableEntity;
  if(category==='classes')return{id,name:'New Class',description:'',tags:[],availability:'NPC',rarity:'common',requirements:[],startingAbilityIds:[],startingSkillModifiers:{},startingModifiers:[],startingEquipment:[],grantedTraits:[],startingEffects:[]} as EditableEntity;
  if(category==='specialPaths')return{id,name:'New Special Path',description:'',tags:[],availability:'NPC',rarity:'rare',unlockConditions:[],progression:[]} as EditableEntity;
  if(category==='namePools')return{id,name:'New Name Pool',description:'',tags:[],firstNames:['Name'],surnames:[]} as EditableEntity;
...
export const EditorApp: React.FC = () => {
  ...
  const filtered = entities.filter((entity) => `${entityName(entity)} ${entity.id} ${entity.tags.join(' ')}`.toLowerCase().includes(search.toLowerCase()) && (category !== 'rooms' || !roomCategoryFilter || (entity as BaseRoomDefinition).roomType === roomCategoryFilter));
  ...
  const duplicateEntity = () => {
    ...
    updateCollection(category, [...entities, { ...structuredClone(selected), id, ...('name' in selected ? { name: `${entityName(selected)} Copy` } : { title: `${entityName(selected)} Copy` }) }]); setSelectedId(id);
  };
  const renameEntity = () => {
    if (!selected) return;
    const name = window.prompt('Entity display name', entityName(selected))?.trim();
    if (!name) return;
    updateCollection(category, entities.map((entity) => entity.id === selected.id ? { ...entity, ...('name' in entity ? { name } : { title: name }) } : entity));
  };
  const deleteEntity = () => {
    ...
    if (!window.confirm(`Delete ${entityName(selected)}?${warning}`)) return;
