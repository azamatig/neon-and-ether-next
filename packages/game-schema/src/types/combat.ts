...
export const AbilityEffectSchema = z.object({
  ...
  defeatType: z.enum(['Lethal', 'NonLethal']).default('Lethal'),
...
export const AbilitySchema = BaseEntitySchema.extend({
  ...
  usableContexts: z.array(z.enum(['Combat', 'Exploration', 'Dialogue'])).default(['Combat']),
  ...
  invalidTargetReason: z.string().optional(),
...
export const StatusEffectDefinitionSchema = BaseEntitySchema.extend({
  ...
  damageDefeatType: z.enum(['Lethal', 'NonLethal']).default('Lethal'),
