...
export const GameplayOutcomeEditor: React.FC<{
  ...
  return <section className="rounded border border-zinc-800 bg-black/20 p-3">
    <header className="mb-2 flex items-center justify-between">
      ...
      <div className="flex items-center gap-1"><PresetControl kind="outcome" value={value ?? { type: 'noPresentation' }} onApply={onChange}/>{value && <button type="button" onClick={() => onChange(undefined)} className="text-rose-400"><Trash2 className="h-3.5 w-3.5"/></button>}</div>
    </header>
    {value ? <>
