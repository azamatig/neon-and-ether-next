/** Development-only Item, NPC, and Enemy content editor. */
import React, { useEffect, useMemo, useState } from 'react';
import { ContentRegistry } from '@neon-ether/game-runtime';
import { EnemySchema, GameContent, ItemSchema, NPCSchema } from '@neon-ether/game-schema';
import { AlertTriangle, CheckCircle2, Copy, Plus, Save, Search, Trash2 } from 'lucide-react';
import { SchemaPropertyEditor } from './components/SchemaPropertyEditor.tsx';

type Category = 'items' | 'npcs' | 'enemies';
type EditableEntity = GameContent[Category][number];

const CATEGORY_LABELS: Record<Category, string> = { items: 'Items', npcs: 'NPCs', enemies: 'Enemies' };
const CATEGORY_SCHEMAS = { items: ItemSchema, npcs: NPCSchema, enemies: EnemySchema } as const;

function nextId(category: Category, entities: EditableEntity[]): string {
  const prefix = category === 'items' ? 'item_editor' : category === 'npcs' ? 'npc_editor' : 'enemy_editor';
...
function createTemplate(category: Category, id: string): EditableEntity {
  ...
  return { id, name: 'New Enemy', description: '', tags: [], factionId: 'Hostile', level: 1, tier: 'Standard', attributes, vitals, aiArchetype: 'MeleeRusher', lootTable: [], grantedXp: 25, bountyCredits: 50, behaviorFlags: [], abilityIds: [], portraitIcon: 'Skull' } as EditableEntity;
}
    ...
    <div className="min-h-screen bg-[#050713] p-4 text-zinc-200 font-mono">
      ...
      <div className="grid min-h-[680px] grid-cols-1 gap-4 lg:grid-cols-[320px_1fr]">
        <aside className="rounded-xl border border-zinc-800 bg-black/40 p-3">
          <div className="mb-3 flex gap-1">{(['items','npcs','enemies'] as Category[]).map((target) => <button key={target} onClick={() => selectCategory(target)} className={`flex-1 rounded border p-2 text-[10px] uppercase ${category === target ? 'border-purple-400 bg-purple-500/15 text-purple-300' : 'border-zinc-800 text-zinc-500'}`}>{CATEGORY_LABELS[target]} <span className="block text-sm">{content?.[target].length ?? 0}</span></button>)}</div>
        ...
        <main className="flex min-h-0 flex-col rounded-xl border border-zinc-800 bg-black/40 p-4">
          {selected ? <><div className="mb-3 flex items-center justify-between gap-2"><div><p className="text-[10px] text-cyan-400">SCHEMA-DRIVEN INSPECTOR // {category.toUpperCase()}</p><h2 className="font-bold text-white">{selected.name}</h2></div><div className="flex gap-2"><button onClick={renameEntity} className="rounded border border-zinc-700 px-3 py-2 text-xs">Rename</button><button onClick={deleteEntity} className="flex items-center gap-1 rounded border border-rose-500/40 px-3 py-2 text-xs text-rose-300"><Trash2 className="h-3 w-3"/> Delete</button></div></div><div className="max-h-[570px] flex-1 overflow-auto pr-2"><SchemaPropertyEditor schema={CATEGORY_SCHEMAS[category]} value={selected} content={content!} onChange={editEntity}/></div>{selectedValidation && !selectedValidation.success && <div className="mt-2 rounded border border-rose-500/30 bg-rose-950/20 p-2 text-[10px] text-rose-300">{selectedValidation.error.issues.slice(0, 5).map((issue) => <div key={`${issue.path.join('.')}-${issue.message}`}>{issue.path.join('.')}: {issue.message}</div>)}</div>}</> : <div className="m-auto text-zinc-600">Select or create an entity.</div>}
