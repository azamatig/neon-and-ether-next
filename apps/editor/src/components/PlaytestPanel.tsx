...
import { TelemetryInspector } from './TelemetryInspector.tsx';
...
export function PlaytestPanel({ content, onClose }: { content: GameContent; onClose: () => void }) {
  ...
  const [tab, setTab] = useState<'tools'|'state'|'factions'|'telemetry'|'log'>('tools');
  ...
  const [seed,setSeed]=useState(controller.getRandomState().initialSeed);
  const checkpoints = [
  ...
  return <div className="fixed inset-0 z-[70] bg-black/90 p-4"><section className="mx-auto flex h-full max-w-7xl flex-col rounded-xl border border-purple-500/40 bg-[#070914] p-4">
    ...
    <nav className="my-3 flex gap-2">{(['tools','state','factions','telemetry','log'] as const).map((value) => <button key={value} onClick={() => setTab(value)} className={`rounded px-4 py-2 text-xs uppercase ${tab===value?'bg-purple-500/20 text-purple-300':'text-zinc-500'}`}>{value}{value==='log'?` (${controller.log.length})`:''}</button>)}</nav>
    <div className="flex-1 overflow-auto">
      {tab === 'tools' && <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
        ...
        <Tool title="Deterministic RNG"><div className="rounded border border-zinc-800 p-2 text-[10px] text-zinc-400">Seed {controller.getRandomState().initialSeed} · State {controller.getRandomState().state} · Draws {controller.getRandomState().draws}</div><input className={select} type="number" value={seed} onChange={event=>setSeed(Number(event.target.value))}/><div className="flex gap-2"><button className={button} onClick={()=>controller.resetRandomSeed(seed)}>Set seed</button><button className={button} onClick={()=>controller.restartWithSeed(seed)}>Replay scenario</button></div></Tool>
      ...
      {tab === 'telemetry' && <TelemetryInspector telemetry={controller.telemetry} onRefresh={()=>refresh((value)=>value+1)}/>}
