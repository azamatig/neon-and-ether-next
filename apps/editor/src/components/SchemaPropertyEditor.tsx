...
function referenceCollection(path: string[], content: GameContent): Array<{ id: string; name: string }> | null {
  ...
  if (/eventId$/i.test(field)) return content.events;
  if (/encounterId$/i.test(field)) return content.encounters;
...
export const VariantListEditor: React.FC<{ kind: 'condition' | 'effect'; value: unknown[]; content: GameContent; path: string[]; onChange: (value: unknown[]) => void }> = ({ kind, value, content, path, onChange }) => {
  ...
  return <div className="space-y-2 rounded border border-zinc-800 bg-black/20 p-2"><div className="flex justify-end"><PresetControl kind={kind === 'condition' ? 'conditions' : 'effects'} value={value as any} onApply={(payload) => onChange([...value, ...payload])}/></div>{value.map((entry, index) => {
...
export const SchemaPropertyEditor: React.FC<SchemaPropertyEditorProps> = ({ schema: source, value, path = [], content, onChange }) => {
  ...
  if (type === 'record') {
    ...
    const valueSchema = schema._def.valueType as SchemaLike;
    const addEntry = () => {
      const requested = window.prompt(`New ${labelFor(field)} key`)?.trim();
      if (!requested || Object.hasOwn(recordValue, requested)) return;
      onChange({ ...recordValue, [requested]: createValue(valueSchema) });
    };
    return <div className="space-y-3 rounded border border-zinc-800 bg-black/20 p-3">{Object.entries(recordValue).map(([key, entry]) => <section key={key} className="rounded border border-zinc-700 p-3"><header className="mb-2 flex items-center justify-between"><span className="text-xs font-bold text-cyan-300">{key}</span><button type="button" onClick={() => { const copy = { ...recordValue }; delete copy[key]; onChange(copy); }} className="text-rose-400"><Trash2 className="h-4 w-4"/></button></header><SchemaPropertyEditor schema={valueSchema} value={entry} path={[...path, key]} content={content} onChange={(next) => onChange({ ...recordValue, [key]: next })}/></section>)}<button type="button" onClick={addEntry} className="flex items-center gap-1 rounded border border-cyan-500/30 px-3 py-2 text-xs text-cyan-300"><Plus className="h-3.5 w-3.5"/> Add {labelFor(field)}</button></div>;
  }
  if (type === 'object') {
