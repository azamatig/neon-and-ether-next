  ...
  return <div className="space-y-4">
    ...
    {view === 'preview' && map ? <div className="min-h-[520px] rounded-xl border border-purple-500/30 bg-[#050713] p-4"><PoiScreen poi={resolvedPreview} map={map} stationedNpcs={stationedNpcs} onReturnToMap={() => setView('edit')} onExecuteAction={() => undefined} onTalkNpc={() => undefined}/></div> : <>
      ...
      <section className="space-y-3 rounded-lg border border-purple-500/25 bg-purple-950/10 p-4">
        ...
        {poi.actions.map((action, index) => <details key={action.id} open={index === 0} className="rounded border border-zinc-700 bg-black/30 p-3"><summary className="cursor-pointer text-xs font-bold text-white">{index + 1}. {action.label}</summary><div className="mt-3 space-y-3"><div className="flex justify-end"><PresetControl kind="poiAction" value={action} onApply={(payload: PoiAction) => updateAction(action.id, { ...payload, id: action.id })}/></div>
