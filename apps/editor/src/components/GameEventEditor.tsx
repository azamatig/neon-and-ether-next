...
export const GameEventEditor: React.FC<{ event: GameEvent; content: GameContent; onChange: (event: GameEvent) => void }> = ({ event, content, onChange }) => {
  ...
  return <div className="space-y-4">
    ...
    <details className="rounded-lg border border-zinc-800 bg-black/20 p-4"><summary className="cursor-pointer text-xs font-bold text-white">Event lifecycle conditions and effects</summary><div className="mt-3 grid gap-3 lg:grid-cols-2">{(['conditions','triggerConditions','availabilityConditions','entryEffects','completionEffects'] as const).map((field) => <label key={field}><span className="mb-1 block text-[10px] uppercase text-zinc-500">{field}</span><SchemaPropertyEditor schema={(GameEventSchemaShape[field] as any)} value={event[field]} content={content} onChange={(value) => onChange({ ...event, [field]: value })}/></label>)}</div><div className="mt-3 grid gap-3 lg:grid-cols-2"><GameplayOutcomeEditor label="Completion outcome / event chaining" value={event.completionOutcome} content={content} fallbackSteps={outcomeSteps} onChange={(completionOutcome) => onChange({ ...event, completionOutcome })}/><GameplayOutcomeEditor label="Explicit safe skip outcome" value={event.skipOutcome} content={content} fallbackSteps={outcomeSteps} onChange={(skipOutcome) => onChange({ ...event, skipOutcome })}/></div></details>
  </div>;
};
