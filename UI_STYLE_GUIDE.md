...
The fullscreen `CharacterCreator` is a pre-session selection UI. Attribute and skill ranges, budgets, age range, portraits, backgrounds, perks, campaign origin, and prologue come from authored `NewGameDefinition`, `BackgroundDefinition`, and `PerkDefinition` content. React submits one `CharacterCreationSelection`; `NewGameInitializer` validates and produces the complete initial `GameState` through existing inventory, effects, conditions, stats, quest, faction, and event abstractions. Cancel never mutates the active session.

## Authored new-game prologue

Character confirmation starts the `NewGameDefinition.startingEventId` through the ordinary `GameEvent` runtime. Fullscreen scene presentation, sequential steps, background-conditioned variants, completion effects, and the final gameplay outcome remain authored Event content. A Skip control is rendered only when the Event declares an explicit `skipOutcome`; skipping executes the same mandatory completion effects and never infers a choice.
