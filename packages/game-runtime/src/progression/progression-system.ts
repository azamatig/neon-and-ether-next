...
export interface ProgressionView { level:number; totalXp:number; currentLevelXp:number; nextLevelXp?:number; xpIntoLevel:number; xpForNextLevel?:number }
...
export class ProgressionSystem {
  ...
  resolvePlayer(state:GameState):ProgressionView {
    const target=state.player,definition=target.progressionDefinitionId?this.content.progressionDefinitions.get(target.progressionDefinitionId):undefined;
    const ordered=[...(definition?.levels??[])].sort((a,b)=>a.totalXp-b.totalXp);
    const current=[...ordered].reverse().find(entry=>entry.level<=target.level);
    const next=ordered.find(entry=>entry.level>target.level);
    const currentLevelXp=current?.totalXp??0;
    return {level:target.level,totalXp:target.experience,currentLevelXp,nextLevelXp:next?.totalXp,xpIntoLevel:Math.max(0,target.experience-currentLevelXp),xpForNextLevel:next?next.totalXp-currentLevelXp:undefined};
  }
}
